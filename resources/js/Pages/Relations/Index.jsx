...
import RelationSearch from "../../Components/Relation/RelationSearch"; // Import komponen search
...
export default function RelationsPage({ auth, relations, flash }) {
  ...
  const [showCancelRequestConfirm, setShowCancelRequestConfirm] = useState(null);
  ...
  // Pending requests states (user's own requests)
  ...
  // Load user's pending requests when Join tab is active
  useEffect(() => {
    if (activeTab === 'join') {
  ...
  const fetchUserPendingRequests = async () => {
    ...
    try {
      const response = await axios.get('/api/user/pending-requests');
      if (response.data.requests && Array.isArray(response.data.requests)) {
        setUserPendingRequests(response.data.requests);
      }
    } catch (err) {
      console.error('Error fetching pending requests:', err);
      setPendingRequestsError('Gagal memuat daftar permintaan');
    } finally {
  ...
  const handleCancelRequest = (request) => {
    router.delete(route('relation-requests.cancel', request.id), {
      onSuccess: () => {
        setShowCancelRequestConfirm(null);
        setSuccessMessage('Permintaan berhasil dibatalkan');
        setShowSuccessToast(true);
        setTimeout(() => setShowSuccessToast(false), 3000);
      ...
      onError: (errors) => {
        setShowCancelRequestConfirm(null);
        setPendingRequestsError(errors.message || 'Gagal membatalkan permintaan');
      }
    });
  };
  ...
  return (
    <>
            ...
            {activeTab === 'join' && (
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 md:gap-6">
                ...
                <PendingRequestsList
                  ...
                  onCancelRequest={setShowCancelRequestConfirm}
                />
              </div>
            )}
          </main>
      ...
      <ConfirmationModal
        show={showCancelRequestConfirm}
        onClose={() => setShowCancelRequestConfirm(null)}
        onConfirm={() => handleCancelRequest(showCancelRequestConfirm)}
        title="Batalkan Permintaan?"
        message={
          <>
            Yakin ingin membatalkan permintaan bergabung ke <strong>"{showCancelRequestConfirm?.relation_name}"</strong>?
          </>
        }
      ...
      <MembersModal
        ...
        onRequestHandled={handleMemberKicked}
      />
